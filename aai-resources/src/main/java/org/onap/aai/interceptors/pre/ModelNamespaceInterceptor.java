...
@ConditionalOnProperty(value = "aai.remove-xmlns.enabled", havingValue = "true")
public class ModelNamespaceInterceptor implements ContainerRequestFilter {
    ...
    public void filter(ContainerRequestContext requestContext) throws IOException {
        if(isModelCreationRequest(requestContext)) {
            Reader xmlReader = new InputStreamReader(requestContext.getEntityStream());
            try {
     ...
     * @throws Exception
     */
    private ByteArrayInputStream removeNameSpace(Reader xmlReader) throws Exception {
    ...
    private boolean isModelCreationRequest(ContainerRequestContext requestContext) {
        return requestContext.getMethod().equals(RequestMethod.PUT.toString())
          && MediaType.APPLICATION_XML.equalsIgnoreCase(requestContext.getMediaType().toString())
          && requestContext.getUriInfo().getRequestUri().getPath().contains("/service-design-and-creation/models/model/");
    }
