...
public class JerseyConfiguration extends ResourceConfig {
    ...
    private <T> void throwIfPriorityAnnotationAbsent(Collection<Class<? extends T>> filters) {
        for (Class filterClass : filters) {
            if (!filterClass.isAnnotationPresent(Priority.class)) {
    ...
    private <T> Comparator<Class<? extends T>> priorityComparator() {
        return comparingInt(c -> c.getAnnotation(Priority.class).value());
    }

    private void logRequests() {
        register(new LoggingFilter(log, ENABLE_RESPONSE_LOGGING));
    }

    private boolean loggingEnabled() {
        return parseBoolean(env.getProperty(LOGGING_ENABLED_PROPERTY));
    }

    private boolean isEnabledByActiveProfiles(AnnotatedElement f) {
        return !f.isAnnotationPresent(Profile.class) || env.acceptsProfiles(f.getAnnotation(Profile.class).value());
    }
}